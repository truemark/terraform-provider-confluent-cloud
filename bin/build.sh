...
#
#	   -asmflags '[pattern=]arg list'     -- arguments to pass on each go tool asm invocation.
...
#		                                        If using the -race flag, the install suffix is automatically set to
#		                                        race or, if set explicitly, has _race appended to it. Likewise for the -msan flag. Using a -buildmode option that requires non-default compile flags has a similar effect.
...
#	   -mod mode                          -- module download mode to use: readonly, vendor, or mod. See 'go help modules' for more.
...
# The argument list may be preceded by a package pattern and an equal sign, which restricts the use of that argument list to the building of
#   packages matching that pattern (see 'go help packages' for a description of package patterns). Without a pattern, the argument list
#   applies only to the packages named on the command line. The flags may be repeated with different patterns in order to specify
#   different arguments for different sets of packages. If a package matches patterns given in multiple flags, the latest match on
#   the command line wins. For example, 'go build -gcflags=-S fmt' prints the disassembly only for package fmt, while
#   'go build -gcflags=all=-S fmt' prints the disassembly for fmt and all its dependencies.
#
# For more about specifying packages, see 'go help packages'. For more about where packages and binaries are installed,
# run 'go help gopath'. For more about calling between Go and C/C++, run 'go help c'.
#
...
#OPTS="$VERBOSE $INSTALL_DEPS $REBUILD $MOD"

sudo go build -v -o $PROVIDER_PROD_NAME

mkdir -p ~/.terraform.d/plugins/${PROVIDER_HOSTNAME}/${PROVIDER_NAMESPACE}/${PROVIDER_NAME}/${PROVIDER_VERSION}/${PROVIDER_OS_ARCH}
mv ${PROVIDER_PROD_NAME} ~/.terraform.d/plugins/${PROVIDER_HOSTNAME}/${PROVIDER_NAMESPACE}/${PROVIDER_NAME}/${PROVIDER_VERSION}/${PROVIDER_OS_ARCH}
